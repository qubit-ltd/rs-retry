...
fn unclassified_application_error_aborts_by_default() {
    let retry = RetryConfig::<&str>::builder().max_attempts(3).build().expect("valid config");
...
fn explicit_retry_fallback_retries_unclassified_errors() {
    let retry = RetryConfig::<&str>::builder().max_attempts(2)
...
fn explicit_rule_keeps_order_and_reason() {
    let retry = RetryConfig::<()>::builder().max_attempts(1)
        .rule(AbortRule)
        .build().expect("valid config");
...
fn clone_shares_callbacks_and_preserves_behavior() {
    ...
    let retry = RetryConfig::<()>::builder().max_attempts(2)
...
fn metadata_split_moves_only_application_error() {
    let retry = RetryConfig::<String>::builder().max_attempts(1)
        .fallback(RetryFallback::Abort)
        .build().expect("valid config");
    let error = Retry::new(&retry).run(|| Err::<(), _>("offline".to_owned())).unwrap_err();
