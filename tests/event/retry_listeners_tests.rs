...
fn test_retry_listeners_default_collection_is_populated_by_builder_callbacks() {
    ...
    let retry = Retry::<&'static str>::builder()
        ...
        .on_failure(move |_failure: &AttemptFailure<&'static str>, context: &RetryContext| {
            failure
                .lock()
        ...
        .on_retry(move |_failure: &AttemptFailure<&'static str>, context: &RetryContext| {
            scheduled
                .lock()
                .expect("retry events should be lockable")
                .push(format!("retry:{}", context.attempt()));
        })
        .on_error(move |_error: &RetryError<&'static str>, context: &RetryContext| {
            error
                .lock()
