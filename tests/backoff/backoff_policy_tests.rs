        ...
        for max in [
            ...
            for sample in [0.0, 0.25, 0.5, 0.75, 1.0] {
                let step = BackoffPolicy::uniform(min, max)
                    ...
                    .start_with_random_source(Arc::new(
                        FixedRetryRandomSource::new(sample),
                    ))
...
fn test_policy_serde_round_trip_preserves_valid_policy() {
    ...
    let decoded: BackoffPolicy =
        from_value(encoded).expect("policy should decode");
...
fn test_final_delay_limit_caps_resolved_delay() {
    ...
    let policy: BackoffPolicy =
        from_value(wire).expect("final limit is supported");
    let mut state = policy.start_with_random_source(Arc::new(
        crate::support::FixedRetryRandomSource::new(1.0),
    ));
    for request in [
...
fn test_delay_limit_serde_roundtrip_and_validation() {
    let policy =
        BackoffPolicy::immediate().limit_delay(Duration::from_nanos(7));
