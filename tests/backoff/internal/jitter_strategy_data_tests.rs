...
fn test_jitter_strategy_data_serializes_variants_and_rejects_invalid_ratio_shapes() {
    let values = [
        ...
        (BackoffPolicy::immediate().with_full_jitter(), json!({ "type": "full" })),
        (
    ...
    for (jitter, expected_error) in [
        ...
        (json!({"type": "full", "ratio": 0.5}), "only valid for bounded jitter"),
        ...
        let error = from_value::<RetryPolicy>(data).expect_err("invalid jitter data must be rejected");
        assert!(
    ...
    let error = from_value::<RetryPolicy>(unknown_tag).expect_err("jitter data must reject unknown tags");
