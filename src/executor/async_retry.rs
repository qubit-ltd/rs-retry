...
// =============================================================================
//! Runtime-independent asynchronous execution facade for the policy-based retry
//! API.
...
/// Runtime-independent async retry execution with explicit timeout controls.
///
/// # Type Parameters
...
///     let execution = AsyncRetry::new(&config);
///     let error = execution
...
impl<'a, E: 'static> AsyncRetry<'a, E> {
        ...
        Fut: Future<Output = Result<T, E>>,
    {
        ...
        self.run_with_default_timer(&default_timer, operation).await
    }

    /// Executes retry controls with a caller-selected default timer and freezes
    /// the final result before completion observers run.
    ///
    /// # Type Parameters
    /// - `T`: Successful value returned to the caller.
    /// - `F`: Operation factory invoked once per admitted attempt.
    ...
    ///
    /// # Parameters
    /// - `default_timer`: Timer used when no timer was injected into this
    ///   facade.
    /// - `operation`: Operation whose errors are classified by the registered
    ///   rules.
    ///
    ...
    pub(crate) async fn run_with_default_timer<T, F, Fut>(
        &self,
        default_timer: &dyn Timer,
        operation: F,
    ...
    ///
    /// # Parameters
    /// - `default_timer`: Timer used when this facade has no injected timer.
    /// - `operation`: Factory creating one future for each admitted attempt.
    ///
    /// # Returns
    /// The successful value or terminal retry error with completion
    /// diagnostics.
    #[allow(
        clippy::result_large_err,
        reason = "the internal helper propagates the lossless public terminal error"
...
async fn sleep(
    ...
    poll_fn(move |context| {
        if cancellation.as_mut().poll(context).is_ready() {
            return Poll::Ready(AsyncBackoffOutcome::Cancelled);
        }
        match timer_future.as_mut().poll(context) {
