...
use super::EffectiveAttemptTimeout;
...
use crate::constants::DEFAULT_RETRY_MAX_TOTAL_ELAPSED;
use crate::constants::DEFAULT_RETRY_WORKER_CANCEL_GRACE_MILLIS;
use crate::constants::KEY_ATTEMPT_TIMEOUT_MILLIS;
use crate::constants::KEY_DELAY;
use crate::constants::KEY_JITTER_FACTOR;
...
impl RetryOptions {
    ...
    pub fn to_policy(&self) -> Result<RetryPolicy, RetryPolicyError> {
        let backoff = match &self.delay {
            ...
            RetryDelay::Random { min, max } => BackoffPolicy::uniform(*min, *max)?,
        ...
        let backoff = match self.jitter {
            ...
            RetryJitter::Factor(factor) => backoff.with_bounded_jitter(factor)?,
        };
        let backoff = match self.retry_after_policy {
            RetryAfterPolicy::Replace => backoff.prefer_retry_after(),
            RetryAfterPolicy::AtLeastConfiguredDelay => backoff.use_retry_after_as_minimum(),
        };
        RetryPolicy::builder()
